...
仓库内的 `服务器生产使用/menus/points/播报商店.yml` 是可直接投放到服务器的 PlayerPoints 播报商店菜单示例。该菜单里的 VIP、VIP+、MVP、MVP+、SVIP 和甜蜜播报商品会在 lore 中显示 `播报示例`，玩家购买前可以直接看到主题实际播报文案的大致效果。

这些商品只发放 DreamKillEcho 的展示型权限，例如 `dreamkillecho.theme.vip`、`dreamkillecho.theme.mvpplus`、`dreamkillecho.theme.svip`、`dreamkillecho.theme.love`，不提供击杀回血、伤害加成、额外收益或免掉落能力。
